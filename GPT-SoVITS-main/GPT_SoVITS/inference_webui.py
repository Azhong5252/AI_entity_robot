...
def sovits(gpt_text):
    ...
    def get_tts_wav(GPT_dropdown,SoVITS_dropdown,ref_wav_path, prompt_text, prompt_language, text, text_language, how_to_cut="不切", top_k=20, top_p=0.6, temperature=0.6, ref_free=False):
        ...
        audio_output_path = os.path.join(os.getcwd(), "Audio",time.strftime("%Y-%m-%d-%H-%M-%S") + ".wav")
        wav_output = (np.concatenate(audio_opt, 0) * 32768).astype(np.int16)
        with open(audio_output_path, 'wb') as f:
            ...
            with wave.open(f, 'wb') as wf:
                ...
                wf.writeframes(wav_output.tobytes())
    ...
    temperature = 1
    ref_text_free= True
    get_tts_wav(GPT_dropdown,SoVITS_dropdown,inp_ref, prompt_text, prompt_language, text, text_language, how_to_cut, top_k, top_p, temperature, ref_text_free)
